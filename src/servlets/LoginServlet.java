...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(accessToken != null && !accessToken.equals(""))
		{
			...
			{
				user = facebookUser.getName();
				pass = "FBUser"+facebookUser.getUsername();
				if(!ProdavacManager.postoji(user))
				{
					ProdavacManager.snimiProdavca(facebookUser.getName().split(" ")[0],
													facebookUser.getName().split(" ")[1],
													"Novi Sad",
													123456789,
													user,pass);
				}
